...
data "http" "cf_ips_v4" {
  ...
  lifecycle {
    postcondition {
      ...
      error_message = "Cloudflare ips-v4 fetch failed or returned an empty body — refusing to apply a security group with an empty/garbage allowlist."
    }
  }
}
...
data "http" "cf_ips_v6" {
  ...
  lifecycle {
    postcondition {
      condition     = self.status_code == 200 && length(trimspace(self.response_body)) > 0
      error_message = "Cloudflare ips-v6 fetch failed or returned an empty body — refusing to apply a security group with an empty/garbage allowlist."
    }
  }
}
