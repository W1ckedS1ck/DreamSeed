...
_cf_zone_id() {
    ...
    [[ -z "${CLOUDFLARE_API_TOKEN:-}" ]] && {
        log "Cloudflare: CLOUDFLARE_API_TOKEN not set"
        return 1
    }
...
update_cloudflare_dns() {
    local domain="$1" ip="$2"
    [[ -z "${CLOUDFLARE_API_TOKEN:-}" ]] && {
        log "Cloudflare DNS: skip (token not set)"
        return 0
    }
    local api_base="https://api.cloudflare.com/client/v4"
    local zone_id
    ...
    IFS='|' read -r count record_id old_ip < <(echo "$existing" | jq -r '.result | length as $c | "\($c)|\(. [0].id // "")|\(. [0].content // "")"' 2>/dev/null) || {
        count=0
        record_id=
        old_ip=
    }
    ...
    if [[ "$count" -gt 0 ]]; then
        [[ "$old_ip" == "$ip" ]] && {
            log "Cloudflare DNS: $domain → $ip (unchanged)"
            return 0
        }
...
delete_cloudflare_dns() {
    local domain="$1"
    [[ -z "${CLOUDFLARE_API_TOKEN:-}" ]] && {
        log "Cloudflare DNS cleanup: skip (token not set)"
        echo "  — Cloudflare DNS: skipped (token not set)"
        return 0
    }
    local api_base="https://api.cloudflare.com/client/v4"
    local zone_id
    ...
    [[ -z "$record_id" ]] && {
        log "Cloudflare DNS cleanup: no A record found for $domain"
        echo "  — Cloudflare DNS: no A record found"
        return 0
    }
...
update_cloudflare_dns_direct() {
    ...
    [[ -z "${CLOUDFLARE_API_TOKEN:-}" ]] && {
        log "Cloudflare direct DNS: skip (token not set)"
        return 0
    }
    local zone_id
    ...
    if [[ "$count" -gt 0 ]]; then
        [[ "$old_ip" == "$ip" ]] && {
            log "Cloudflare direct DNS: $subdomain → $ip (unchanged)"
            return 0
        }
        if _cfcurl --retry 1 --retry-delay 3 -X PUT "$base/$record_id" \
            --config <(_bearer_auth "$CLOUDFLARE_API_TOKEN") \
