...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            prod | dev-aws | dev-hetz | prod-hetz | test)
                TARGET="$1"
                shift
                ;;
            -n)
            ...
            -i | --ip)
                if [[ -z "${2:-}" || "$2" =~ ^- ]]; then
                    echo "Error: -i requires an IP address argument"
                    usage
                    exit 1
                fi
                EXISTING_IP="$2"
    ...
    if [[ -z "$WEB_SERVER" && "$DESTROY_MODE" == "false" ]]; then
        echo "Error: web server required (-n/-a)"
        usage
        exit 1
    fi
}
...
resolve_target() {
    ...
    case "$TARGET" in
        ...
        prod-hetz)
            TF_PROVIDER="hetzner"
            DEPLOY_DOMAIN="dreamseed.online"
            TF_WORKSPACE="prod-hetz"
            TARGET_PREFIX="PROD_HETZ"
            SSH_ATTEMPTS=90
            SSH_INTERVAL=2
            ;;
        ...
        test)
            TF_PROVIDER="hetzner"
            DEPLOY_DOMAIN="test.vitalikuts.online"
            TF_WORKSPACE="test"
            TARGET_PREFIX="DEV_HETZ"
            SSH_ATTEMPTS=90
            SSH_INTERVAL=2
            ;;
        *)
            echo "Error: unknown target '$TARGET'. Valid: prod, dev-aws, dev-hetz, prod-hetz, test"
            exit 1
            ;;
    esac
