...
# HTTP→HTTPS redirect
if [[ -n "$DOMAIN" ]]; then
  _http_code=$(curl -sI -o /dev/null -w '%{http_code}' --max-time 5 "http://${DOMAIN}/" 2>/dev/null || echo "000")
  ...
  # Site response
  _site_ttfb=$(curl -sk -o /dev/null -w '%{http_code} (%{time_total}s)' --max-time 10 "https://${DOMAIN}/" 2>/dev/null || echo "error")
  echo "    Site: $_site_ttfb"
  # 404 handling
  _404_code=$(curl -sk -o /dev/null -w '%{http_code}' --max-time 5 "https://${DOMAIN}/nonexistent" 2>/dev/null || echo "000")
...
_site=$(curl -sk -o /dev/null -w 'HTTP %{http_code} | TTFB: %{time_starttransfer}s | Total: %{time_total}s' --max-time 10 "https://${DOMAIN}/" 2>/dev/null || echo "error")
echo "    Site: $_site"

_static=$(curl -sk -o /dev/null -w 'HTTP %{http_code} | TTFB: %{time_starttransfer}s | Total: %{time_total}s' --max-time 10 "https://${DOMAIN}/theme/css/style.css" 2>/dev/null || echo "error")
echo "    Static: $_static"
...
crontab -l 2>/dev/null | grep "^[0-9]" | while read -r _c; do echo "    $_c"; done
