...
## Fail2ban web-jail whitelist (per target)

```
FAIL2BAN_IGNOREIP_DEV_AWS="93.125.0.0/16 178.125.0.0/16"
FAIL2BAN_IGNOREIP_PROD_HETZ=""        # empty by default
FAIL2BAN_IGNOREIP="..."               # generic fallback for any target
```

Space-separated IPs/CIDRs that the web jails (modx-admin, dreamseed-botsearch,
dreamseed-bad-request) never ban — for developers / AI agents behind dynamic
ISP addresses who would otherwise get Cloudflare-edge-banned during normal
manager work. sshd is NOT whitelisted. Stored in `secrets/.env`; picked up by
`lib/gen_vars.py` → `fail2ban_ignoreip` extra var.
