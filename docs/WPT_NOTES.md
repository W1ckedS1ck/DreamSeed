...
> Июль 2026. Все, что не влезло в инфраструктурный код.
...
**Фикс**: вынести CSS в `[[$css]]`, JS в `[[$js]]` — вызывать **1 раз** в основном шаблоне. `[[$head]]` оставить только для `<meta>`, `<title>`, шрифтов.
...
`/theme/images/favicon2.png` (2.7 KB, можно до 1 KB). Конвертнуть в `.webp` или прогнать через ImageOptim/optipng.

## 🔲 Cloudflare Dashboard (не terraform)

### Email Obfuscation
Текущий CF API токен не имеет прав `zone:settings:edit`. Если хочешь через код — нужно расширить токен.
