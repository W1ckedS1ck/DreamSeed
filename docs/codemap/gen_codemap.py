...
NODES = [
    ...
    {"id": "ext-betterstack", "type": "external", "path": "scripts/setup_betteruptime.sh",
         ...
         "tests": [],
         "constraints": ["Each cron script pings its own heartbeat key on success only (smart_backup.sh:168-176)", "Heartbeats/monitors auto-created via setup_betteruptime.sh --write-env (lib/preflight.sh:33)", "setup_betteruptime.sh re-encrypts secrets/.env with ansible-vault when --write-env (setup_betteruptime.sh:75)", "prod preflight reconciles heartbeat period/grace on every deploy via setup_betteruptime.sh --heartbeats-only"],
