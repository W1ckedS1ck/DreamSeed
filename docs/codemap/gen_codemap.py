...
NODES = [
    ...
    {"id": "deploy", "type": "orchestrator", "path": "deploy.sh",
         ...
         "constraints": ["Prod destroy requires interactive 3-step confirm or CI_DESTROY_CONFIRM=yes (lib/terraform.sh:45)", "Per-target flock lock prevents concurrent deploys (deploy.sh:159)", "Playbooks run in phases: 01 -> 02+03+09 -> 04 -> 05+06+08+07 (07 = non-fatal fallback)", "Sources lib/*.sh at startup — never exec'd directly (deploy.sh:41-45)"],
         "evidence": [{"path": "deploy.sh", "symbol": "main"}, {"path": "deploy.sh", "symbol": "resolve_target"},
                   {"path": "lib/cli.sh", "symbol": "run_lint"}, {"path": "lib/cli.sh", "symbol": "parse_args"},
    ...
    {"id": "ansible-roles-db", "type": "ansible", "path": "ansible-roles/mariadb",
         ...
         "evidence": [{"path": "ansible-roles/mariadb/tasks/main.yml", "symbol": "ansible.mariadb.mariadb_user"},
                   {"path": "ansible-roles/mariadb/tasks/main.yml", "symbol": "ansible.mariadb.mariadb_db"},
    ...
    {"id": "ansible-roles-security", "type": "ansible", "path": "ansible-roles/security",
         ...
         "role": "Security hardening: sshd hardening + sshd -t validation, fail2ban jails (modx-admin 1000 failures/10min -> 1h ban, no Referer/UA exemption, botsearch, bad-request), sysctl hardening, restrictive cron/sshd file permissions.",
    ...
    {"id": "ansible-roles-observability", "type": "ansible", "path": "ansible-roles/monitoring",
         ...
         "tests": [".github/workflows/test-restore.yml"],
         "constraints": ["GRAFANA_CLOUD_URL must be regional prometheus-* URL (lib/preflight.sh:58-68)", "vmagent metric token must be glc_* (Cloud Access Policy), not glsa_* (lib/preflight.sh:69-74)", "vm_retention=3mo (group_vars/all.yml:69)", "Grafana alerts send to Telegram contact point (grafana-alerts.yaml.j2:54-61)"],
    ...
    {"id": "scripts-server", "type": "scripts", "path": "scripts/common_functions.sh",
         ...
         "constraints": ["Backup locked via flock $HOME/.locks/smart_backup.lock (smart_backup.sh:50-59)", "RESTORE_ALL --auto-latest forces prod cloud paths (RESTORE_ALL.sh)", "Cron scripts push metrics to local VictoriaMetrics :8428 and ping per-script Better Stack heartbeats on success", "rclone_retry uses RCLONE_CMD_TIMEOUT env var, not RCLONE_TIMEOUT (CLAUDE.md)"],
    ...
    {"id": "runtime-db-cache", "type": "runtime", "path": "ansible-roles/redis",
         ...
         "constraints": ["Redis single-server bind 127.0.0.1, no auth (group_vars/all.yml:27-29)", "mysqldump uses .my.cnf — no credentials on CLI (smart_backup.sh:162-169)", "MODX session table excluded from DB dump (smart_backup.sh:163)"],
         "evidence": [{"path": "ansible-roles/mariadb/tasks/main.yml", "symbol": "ansible.mariadb.mariadb_db"},
    ...
    {"id": "ext-betterstack", "type": "external", "path": "scripts/setup_betteruptime.sh",
         ...
         "tests": [],
         "constraints": ["Each cron script pings its own heartbeat key on success only (smart_backup.sh:235-240)", "Heartbeats/monitors auto-created via setup_betteruptime.sh --write-env (lib/preflight.sh:33)", "setup_betteruptime.sh re-encrypts secrets/.env with ansible-vault when --write-env (setup_betteruptime.sh:75)", "prod preflight reconciles heartbeat period/grace on every deploy via setup_betteruptime.sh --heartbeats-only"],
         "evidence": [{"path": "scripts/common_functions.sh", "symbol": "ping_heartbeat"},
                   {"path": "scripts/setup_betteruptime.sh", "symbol": "heartbeat_lookup"},
...
EDGES = [
    ...
    {"from_id": "github-ci", "to": "ansible-playbooks", "type": "calls",
         "evidence": [{"path": ".github/workflows/test-restore.yml", "symbol": "deploy.sh test -n -p"}],
    ...
    {"from_id": "lib", "to": "ansible-playbooks", "type": "calls",
         "evidence": [{"path": "lib/ansible.sh", "symbol": "_ansible_cmd"},
                   {"path": "lib/ansible.sh", "symbol": "run_parallel"}],
    ...
    {"from_id": "deploy", "to": "tooling", "type": "calls",
         "evidence": [{"path": "lib/cli.sh", "symbol": "run_lint"}],
    ...
    {"from_id": "github-ci", "to": "terraform-saas", "type": "calls",
         "evidence": [{"path": ".github/workflows/drift-detection.yml", "symbol": "tf_dir: terraform/cloudflare"},
                   {"path": ".github/workflows/terraform-apply.yml", "symbol": "Set provider env vars"},
                   {"path": ".github/workflows/grafana-cloud.yml", "symbol": "terraform apply -auto-approve"}],
...
FLOWS = [
    {"id": "flow-deploy", "name": "Deploy environment (provision -> configure -> go-live)",
         ...
         "outcome": "Server provisioned on AWS/Hetzner, configured by 9 playbooks, DNS A record points domain -> server, check_services passes, Telegram 'Deploy complete' notification."},
...
function fitView() {
  ...
  for (const n of DATA.nodes) {
    const g = nodeG[n.id];
    const b = g ? g.getBBox() : { x: -34, y: -34, width: 68, height: 68 };
    minX = Math.min(minX, n.x + b.x); maxX = Math.max(maxX, n.x + b.x + b.width);
    minY = Math.min(minY, n.y + b.y); maxY = Math.max(maxY, n.y + b.y + b.height);
  }
  const r = svg.getBoundingClientRect();
  const pad = 16;
  const k = Math.max(0.25, Math.min(1.4, Math.min((r.width - pad*2)/(maxX-minX), (r.height - pad*2)/(maxY-minY))));
  const cx = (minX+maxX)/2, cy = (minY+maxY)/2;
  view = { x: r.width/2 - cx*k, y: r.height/2 - cy*k, k };
  applyView();
}

const nodeG = {}, edgeG = {};
for (const e of edges) {
  const g = document.createElementNS('http://www.w3.org/2000/svg','g'); g.setAttribute('class', 'edge');
